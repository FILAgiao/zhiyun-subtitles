...
(async()=>{
 ...
 await page.evaluate(()=>{document.querySelector('#zy-subtitle-host').shadowRoot.activeElement?.blur();document.activeElement?.blur();});
 await page.keyboard.press('ArrowRight');assert.equal(await page.evaluate(()=>window.videoTime),22);
 await page.keyboard.press('ArrowLeft');assert.equal(await page.evaluate(()=>window.videoTime),12);
 await ui.locator('#cache-seek').evaluate(e=>{e.value='45';e.dispatchEvent(new Event('input',{bubbles:true}));});assert.equal(await page.evaluate(()=>window.videoTime),45);
 await ui.locator('#cache-back').click();assert.equal(await page.evaluate(()=>window.videoTime),35);
 await page.evaluate(()=>window.videoTime=12);
 ...
 await ui.locator('summary').filter({hasText:'视频缓存'}).click();
 const thirdRow=ui.locator('#batch-list > div').filter({hasText:'课次 3'});
 await thirdRow.getByRole('button',{name:'删除缓存',exact:true}).click();await page.waitForTimeout(150);
 assert.match(await thirdRow.textContent(),/缓存已删除/);
 await thirdRow.getByRole('button',{name:'移出清单',exact:true}).click();await page.waitForTimeout(150);
 assert.equal(await thirdRow.count(),0);
 await ui.locator('#batch-refresh').click();await page.waitForTimeout(100);assert.equal(await thirdRow.count(),0);
 await ui.locator('summary').filter({hasText:'视频缓存'}).click();
