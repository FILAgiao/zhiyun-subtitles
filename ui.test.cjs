...
(async()=>{
 ...
 // Split equally, resize/drag only inside the PPT half, restore video width on close.
 ...
 const moved=await ui.locator('#slides').boundingBox();assert.ok(moved.x>resized.x+60);assert.ok(moved.x+moved.width<=videoBefore.x+1);
 ...
 const fsVideo=await page.locator('video').boundingBox(),fsSlides=await ui.locator('#slides').boundingBox();assert.equal(fsVideo.width,720);assert.ok(fsSlides.x+fsSlides.width<=fsVideo.x+1);
