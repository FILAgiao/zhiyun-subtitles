...
(async()=>{
 ...
 // Resize the whole floating window, then drag it; neither operation changes image zoom.
 const before=await ui.locator('#slides').boundingBox();
 const grip=await ui.locator('#slide-resize').boundingBox();
 await page.mouse.move(grip.x+12,grip.y+12);await page.mouse.down();await page.mouse.move(grip.x+112,grip.y+72,{steps:5});await page.mouse.up();
 const resized=await ui.locator('#slides').boundingBox();assert.ok(resized.width>before.width+80);assert.ok(resized.height>before.height+40);
 await page.mouse.move(resized.x+100,resized.y+50);await page.mouse.down();await page.mouse.move(resized.x+180,resized.y+90,{steps:5});await page.mouse.up();
 const moved=await ui.locator('#slides').boundingBox();assert.ok(moved.x>resized.x+60);
