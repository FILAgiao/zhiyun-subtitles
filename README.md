...
打开 PPT 后恢复左右分区：左侧 PPT、右侧视频各占一半。初次打开时 PPT 位于左侧；之后可跨越分区自由拖动和缩放，记住用户调整后的布局，允许部分移出屏幕，至少保留 72 像素可见区域。关闭后视频恢复原宽度。

翻译前先进行本地语言检测：明确属于目标语言的字幕不发送给豆包，也不重复显示译文。中文为主且夹杂少量英文术语的句子通常跳过英译中；包含完整英文句子的混合字幕仍翻译。检测基于文字规则，极短或复杂混合语言可能判断不准。

左右拖边缘只改变窗口宽度，裁掉左右黑边或多余画面；上下拖边缘改变显示高度，PPT 按比例放大。图片不拉伸，超出窗口的部分会裁切。初次打开按图片比例完整显示。「复位 PPT」可恢复默认位置和大小。
...
3. 刷新智云课堂回放，打开右侧「语音识别」。面板版本应为 **v0.10.2**。
