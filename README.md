...
3. 刷新智云课堂回放，打开右侧「语音识别」。面板版本应为 **v0.9.0**。
...
拖动 PPT 图片移动窗口，拖右下角缩放。悬停或键盘聚焦时显示翻页与跟随按钮。手动翻页暂停自动跟随，再次打开保留页码；开启跟随时按当前视频时间同步。
...
`npm test` 运行无外部依赖的 22 项单元测试。UI 测试使用模拟课堂、虚拟凭证和模拟接口，验证字幕状态、PPT 翻页与跟随、窗口拖拽缩放、全屏、进度和取消，不调用真实云端服务。测试生成 `ui-preview.png`。
