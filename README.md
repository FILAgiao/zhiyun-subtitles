...
已将用户确认的 `vod.cmc.zju.edu.cn` 加入油猴元数据及下载域名校验。更新必须完整替换脚本（包含顶部权限），刷新课程后允许油猴连接该域名。仍保留 512 MB 上限及 MP4/WebM 格式限制。
...
- 当前只支持地址明确为 MP4/WebM、位于已授权域名且当前登录可访问的文件，单个最多 **512 MB**。浏览器存储空间不足时会报错。
...
3. 刷新智云课堂回放，打开右侧「语音识别」。面板版本应为 **v0.11.2**。
