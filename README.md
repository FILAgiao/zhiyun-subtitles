...
26 项单元测试及模拟浏览器测试覆盖：网页 fetch 报 CORS 错误时仍走油猴下载完成缓存、域名限制、文件头、取消、大小限制。浏览器控制工具启动失败，尚未在用户当前课堂及真实油猴扩展中验收。
...
3. 刷新智云课堂回放，打开右侧「语音识别」。面板版本应为 **v0.11.1**。
...
`npm test` 运行无外部依赖的 26 项单元测试。UI 测试使用模拟课堂、虚拟凭证和模拟接口，验证字幕状态、PPT 翻页与跟随、窗口拖拽缩放、全屏、进度和取消，不调用真实云端服务。测试生成 `ui-preview.png`。
