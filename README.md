...
打开 PPT 后恢复左右分区：左侧 PPT、右侧视频各占一半。PPT 可在左半区内拖动和缩放，关闭后视频恢复原宽度，全屏仍保持分区。
...
- 采集 12 秒视频音轨自动匹配字幕时间，匹配不可靠时最多尝试三段；可查看进度、随时取消。
...
3. 刷新智云课堂回放，打开右侧「语音识别」。面板版本应为 **v0.10.0**。
...
拖动 PPT 图片在左半区移动，拖右下角缩放，最大不超过左半区。悬停或键盘聚焦时显示翻页与跟随按钮。手动翻页暂停自动跟随，再次打开保留页码；开启跟随时按当前视频时间同步。
